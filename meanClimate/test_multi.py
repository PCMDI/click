...
pmp_run = 'cmec_10262017'
pmp_run = 'cmec_11022017'

exp = 'amip'
exp = 'historical'
#exp = 'picontrol'

injpath = '/work/gleckler1/processed_data/metrics_package/metrics_results/cmip5clims_metrics_package-' + exp + '/' + pmp_run + '/'
...
if stat == 'rms_xy': 
  tit = 'Seasonal RMS (' + stat + ')'
if stat == 'rms_devzm_xy': 
  tit = 'Seasonal RMS: Deviations from zonal mean (' + stat + ')' 
if stat == 'rms_devam_xy':
  tit = 'Seasonal RMS: Deviations from annual mean (' + stat + ')'
...
if stat == 'rms_xyt': rms_xy = J(statistic=["rms_xyt"],region="global")(squeeze=1) 
...
if stat != 'rms_xyt': mesh, template, meshfill = P.plot(rms_xy[...,0],x=x)
if stat == 'rms_xyt': mesh, template, meshfill = P.plot(rms_xy,x=x)
...
x.png(pathout + click_name + ".png")

# Creates clickable polygons numpy arrays
click_areas = vcs.utils.meshToCoords(mesh,template,[meshfill.datawc_x1,meshfill.datawc_x2,meshfill.datawc_y1,meshfill.datawc_y2],png= pathout + click_name + ".png")
...
if exp == 'amip': targets_template = genutil.StringConstructor("plots/cmip5/amip/%(stat)/%(variable)/%(variable).%(model)_%(season).png")
if exp == 'historical': targets_template = genutil.StringConstructor("plots/cmip5/historical/%(stat)/%(variable)/%(variable).%(model)_%(season).png")
...
def createHtml(target,json_file,exp,stat,sanitize_json=True):
    with open(json_file) as f:
        jsn = json.load(f)
    fname = "{}_{}_{}.html".format(target.variable,target.model,target.season)
...
#       print "After sanitize:",r.keys()
        ...
        json_file = os.path.join(pathout,"htmls","cmip5",exp,stat,mod + '_' + os.path.basename(json_file))
...
#   print "NEW JSON FILE:",json_file
    ...
    targ = '../../../../' + target()
    fname2 = os.path.join(pathout,"htmls","cmip5",exp,stat,fname)  ##PJG add exp and stat
...
#       print>>f,"<h2>Model:{}<br>Variable: {}<br>Season {}".format(target.model,target.variable,target.seasonj
        print>>f,"<a href='{}'>Return to roadmap".format(os.path.join("..","..","..","../",click_name + '.html')) 
        print>>f,"<div><img  width = '100' height = '100' src='{}'></div>".format(os.path.join("..","..","..","..",click_name + ".png"))
        print>>f,"<div><center><img src='{}'></center></div></a>".format(os.path.join("..","..","..","../",target()))
#       print>>f,"<h3>Additional statistics</h3>"
        print>>f,"<a href='{}'><h3><center>Detailed statistics and provenance</center></h3></a>".format('./' + os.path.basename(json_file))
...
def createTargets(data,targets_template, season,exp,stat):
    ...
    if stat == 'rms_xy': clm = 'clim' 
    if stat == 'rms_devzm_xy':clm = 'climmzm'
    if stat == 'rms_devam_xy': clm = 'climmtm'
    ...
    for a in data.getAxis(0).id.split("___"):
        ...
        for b in data.getAxis(-1).id.split("___"):
            ...
            # Here we test if 
            tipsString = "Model: %s<br>Season: %s<br>Variable: %s<br>Value: %.3g<div id='thumbnail'><img src='%%s' width=200></div>" % (b,season,a,flt[indx])
...
#targets, tooltips = createTargets(rms_xy[...,0],targets_template)

###
#x.clear()
#seasons = J.getAxis("season")
...
for i, ss in enumerate(zrange):
    if stat in ['rms_devam_xy','rms_devzm_xy']: multi= i + 1.2
    if stat == 'rms_xy': multi=i + 1.4 
    mesh, template, meshfill = P.plot(rms_xy[...,i+1],x=x,multiple=multi)
    x.png(pathout + click_name + ".png")
    click_areas1 = vcs.utils.meshToCoords(mesh,template,[meshfill.datawc_x1,meshfill.datawc_x2,meshfill.datawc_y1,meshfill.datawc_y2],png=pathout + click_name + ".png")

    it = i
    if stat in ['rms_devam_xy','rms_devzm_xy'] and i == 1: it = 2  
    targets1, tooltips1 = createTargets(rms_xy[...,it+1],targets_template, seasons[it+1],exp,stat)
    if i==0:
...
share_pth = "."  # pathout
with open(pathout + click_name + ".html","w") as f:
    f.write("<html><head>")
    ...
    f.write("<div><img  width = '700' height = '700' src='ParallelCoordinates/Parallel_Plot_" + exp + "_djf.png'></div>")
    f.write("<div><img  width = '700' height = '700' src='ParallelCoordinates/Parallel_Plot_" + exp + "_mam.png'></div>")
    f.write("<div><img  width = '700' height = '700' src='ParallelCoordinates/Parallel_Plot_" + exp + "_jja.png'></div>")
    f.write("<div><img  width = '700' height = '700' src='ParallelCoordinates/Parallel_Plot_" + exp + "_son.png'></div>")
